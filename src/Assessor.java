 ...
 * Group: 1
 */
public class Assessor extends JPanel implements ActionListener, ItemListener, FocusListener {
    ...
    public Assessor(Universe frame)
    {
        super();
        parentFrame = frame;
    ...
    public void actionPerformed(ActionEvent e)
    {
        ...
        if (e.getSource() == submit)
        {
            if (button1.isSelected() == true)
            {
                if(position ==1)
                {
                    Descriptor.appendText(true);
                    correctCount++;
                    setQuestions(correctCount);
                }
                else
                {
                    Descriptor.appendText(false);
                    wrongCount++;
                    setQuestions(correctCount);
                }
